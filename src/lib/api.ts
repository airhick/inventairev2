...
function normalizeItem(raw: any): Item {
  return {
    ...
    hasImage: !!raw?.hasImage || !!raw?.image, // Rétrocompatibilité
    imageTimestamp: Number(raw?.imageTimestamp || 0),
  };
}
...
export async function getItems(): Promise<Item[]> {
  ...
  // Backfill des anciens items qui n'ont pas de itemId et migration des images.
  for (const row of sorted) {
    let needsUpdate = false;
    let updates: Partial<Item> = {};

    // 1. Génération de itemId manquant
    if (!row.item.itemId) {
      ...
      updates.itemId = generated;
      needsUpdate = true;
    }

    // 2. Migration silencieuse des grosses images base64 vers /itemImages
    if (row.item.image && row.item.image.length > 200) {
      const isBase64 = row.item.image.startsWith('data:image/');
      if (isBase64) {
        // Sauvegarde dans le nouveau noeud
        await saveItemImageToNode(row.item.serialNumber, row.item.image);
        // On nettoie l'image de l'objet principal
        updates.image = '';
        updates.hasImage = true;
        updates.imageTimestamp = Date.now();

        row.item.image = '';
        row.item.hasImage = true;
        row.item.imageTimestamp = updates.imageTimestamp;
        needsUpdate = true;
      }
    }
    ...
    if (needsUpdate) {
      ...
      await update(dbRef(rtdb, `items/${row.key}`), sanitizeForRtdb(updates));
    }
  }
...
export async function saveItemImageToNode(serialNumber: string, base64Image: string | null): Promise<void> {
  if (!serialNumber) return;
  const imageRef = dbRef(rtdb, `itemImages/${serialNumber}`);
  if (base64Image) {
    await set(imageRef, base64Image);
  } else {
    await remove(imageRef);
  }
}

export async function getItemImageFromNode(serialNumber: string): Promise<string | null> {
  if (!serialNumber) return null;
  const snap = await get(dbRef(rtdb, `itemImages/${serialNumber}`));
...
export async function saveItem(itemData: Partial<Item>): Promise<any> {
  ...
  let nextItemId = itemData.itemId?.trim()
    ? itemData.itemId.trim().toUpperCase()
    : await getNextFormattedItemId();

  let autoParentId = itemData.parentId ?? null;
  let autoDisplayOrder = Number(itemData.displayOrder || 0);

  // Recherche d'un parent existant si on n'a pas forcé de parentId
  if (autoParentId === null) {
    const existingItems = await getAllArray<Item>('items');

    // Critères de recherche : itemId, ou code scanné
    const searchItemId = itemData.itemId?.trim().toUpperCase();
    const searchCode = (itemData.scannedCode || itemData.barcode)?.trim().toLowerCase();

    // Trouver le "master" (idéalement un item qui n'a pas de parent, ou le premier trouvé)
    const matchingParent = existingItems.find(it => {
      // S'il a déjà un parent, on l'ignore car on veut se lier au maître
      if (it.parentId !== null && it.parentId !== undefined) return false;

      const matchId = searchItemId && it.itemId?.toUpperCase() === searchItemId;
      const matchCode = searchCode &&
        (it.scannedCode?.toLowerCase() === searchCode || it.barcode?.toLowerCase() === searchCode);

      return matchId || matchCode;
    });

    if (matchingParent) {
      console.log(`[Auto-Hierarchy] Item existant trouvé (${matchingParent.id} - ${matchingParent.name}). Liaison en cours...`);
      autoParentId = matchingParent.id!;
      // On hérite aussi de son itemId visuel s'il en a un
      if (matchingParent.itemId) {
        nextItemId = matchingParent.itemId;
      }
      // On le place à la fin (ou on estime un ordre basé sur les enfants existants)
      const childrenCount = existingItems.filter(it => it.parentId === matchingParent.id).length;
      autoDisplayOrder = autoDisplayOrder || (childrenCount + 1);
    }
  }

  // Extraction de l'image pour le stockage séparé
  const hasImageBase64 = !!itemData.image && itemData.image.length > 200; // rough check for base64
  ...
  const payload: Item = {
    ...
    // Si c'est une base64, on ne la met pas inlined. Sinon on garde l'URL
    image: hasImageBase64 ? '' : (itemData.image || ''),
    ...
    lastUpdated: nowIso(),
    // Cache management
    hasImage: hasImageBase64 || !!itemData.image,
    imageTimestamp: hasImageBase64 ? Date.now() : 0,
  };

  // 1. Sauvegarder l'item principal
  await set(dbRef(rtdb, `items/${id}`), sanitizeForRtdb(payload));

  // 2. Sauvegarder l'image séparément si fournie en base64
  if (hasImageBase64) {
    await saveItemImageToNode(payload.serialNumber, itemData.image!);
  }
...
export async function updateItem(serialNumber: string, updates: Partial<Item>): Promise<any> {
  ...
  const itemUpdates = { ...updates, lastUpdated: nowIso() };

  // Gestion de l'image (si on en fournit une nouvelle)
  if (itemUpdates.image !== undefined) {
    const isBase64 = itemUpdates.image && itemUpdates.image.length > 200;

    if (isBase64) {
      await saveItemImageToNode(serialNumber, itemUpdates.image!);
      itemUpdates.image = ''; // Remove from payload to keep /items light
      itemUpdates.hasImage = true;
      itemUpdates.imageTimestamp = Date.now();
    } else if (itemUpdates.image === null || itemUpdates.image === '') {
      await saveItemImageToNode(serialNumber, null); // Delete remote image
      itemUpdates.hasImage = false;
      itemUpdates.imageTimestamp = Date.now();
    } else {
      // It's a regular small string (like an external URL)
      itemUpdates.hasImage = true;
      itemUpdates.imageTimestamp = Date.now();
    }
  }
  ...
  await update(
    ...
    sanitizeForRtdb(itemUpdates),
  );
  await pushNotification({
    message: `Item modifie: ${itemUpdates.name || serialNumber}`,
    type: 'item_updated',
    itemSerialNumber: itemUpdates.serialNumber || serialNumber,
    itemHexId: itemUpdates.hexId || found.item.hexId,
  });
  return { success: true };
}
...
export async function deleteItem(serialNumber: string): Promise<any> {
  ...
  await remove(dbRef(rtdb, `itemImages/${serialNumber}`)); // On nettoie l'image séparée aussi
  await pushNotification({
...
export async function deleteAllItems(): Promise<{ success: boolean; count: number }> {
  ...
  await set(dbRef(rtdb, 'itemImages'), {}); // On vide aussi toutes les images stockées séparément

  // Si on est côté client, on vide aussi le cache local
  if (typeof window !== 'undefined') {
    try {
      const { clearImageCache } = await import('./imageCache');
      await clearImageCache();
    } catch (e) { console.error('Error clearing local cache', e); }
  }
